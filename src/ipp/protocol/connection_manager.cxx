	...
	namespace protocol {
		...
		connection& connection_manager::connect(const network::socket_address& addr) {
			...
			if (iterator == _connection_map.end()) {
				connection new_connection{_sock.connect(addr), _listener};
		...
		void connection_manager::consume_socket() {
			if (_sock.listening() && _sock.accpetable(std::chrono::milliseconds::zero())) {
				...
				_connection_map.emplace(std::piecewise_construct, std::forward_as_tuple(std::move(address)),
				                        std::forward_as_tuple(std::move(sock_con), _listener));
			}
