...
namespace ipp {
	namespace protocol {
		...
		public:
			...
			using connection_map_type = std::unordered_map<network::socket_address, std::unique_ptr<connection>>;
			...
			const std::unique_ptr<connection>& connect(const network::socket_address&);
			void consume_socket();
